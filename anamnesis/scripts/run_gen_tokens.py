...
def _setup_injection(model, inject_npz, inject_key, inject_layer, inject_alpha,
                     inject_alpha_frac, label):
    """Attach the residual-write hook for one cell; return (write_handle, inject_meta).

    Returns (None, None) when inject_npz is None. Identical to the original inline
    setup — factored so the single-cell path is byte-for-byte unchanged and the
    multi-cell path can re-attach per cell (removing the previous handle first)."""
    if inject_npz is None:
        return None, None
    if inject_key is None or inject_layer is None or inject_alpha is None:
    ...
    vec_bank = np.load(inject_npz)
    if inject_key not in vec_bank:
        raise SystemExit(f"vector key {inject_key!r} not in {inject_npz} "
                         f"(has {list(vec_bank.keys())})")
    vec = torch.from_numpy(vec_bank[inject_key].astype(np.float32))
    spec = ResidualWriteSpec(layer_idx=int(inject_layer), vector=vec,
                             alpha=float(inject_alpha), start_pos=None, normalize=True)
    write_handle = attach_residual_write(model, spec)
    inject_meta = {"inject_npz": str(inject_npz), "inject_key": inject_key,
                   "inject_layer": int(inject_layer), "inject_alpha": float(inject_alpha),
                   "inject_alpha_frac": inject_alpha_frac}
    logger.info(f"[{label}] steered generation: {inject_meta}")
...
def _generate_specs(model, tok, pad_id, specs, out_dir: Path, write_handle, inject_meta,
    ...
    todo = [s for s in specs if not (out_dir / f"gen_{s['generation_id']:03d}.json").exists()]
    ...
    for i, spec in enumerate(todo):
        try:
            ...
            if tok.chat_template is None:
                ...
                template_kwargs = {"date_string": gp["date_string"]} if gp["date_string"] else {}
            ...
            record = {
                "generation_id": gid, "prompt_set": spec["prompt_set"],
                "topic": spec["topic"], "topic_idx": spec["topic_idx"],
                "mode": spec["mode"], "mode_idx": spec["mode_idx"],
                "system_prompt": spec["system_prompt"], "user_prompt": spec["user_prompt"],
                "seed": spec["seed"], "repetition": spec["repetition"],
                "condition": "standard", "generated_text": generated_text,
            ...
            n_fail += 1
            logger.error(f"[{label}] spec {spec.get('generation_id')} FAILED: {exc}",
                         exc_info=True)
...
def main() -> None:
    ...
    ap.add_argument("--spec-file", type=Path, default=None, help="JSON list of generation specs (single-cell mode)")
    ap.add_argument("--out-dir", type=Path, default=None, help="Where to write per-gen token records (single-cell mode)")
    ap.add_argument("--jobs-file", type=Path, default=None,
                    help="MULTI-CELL mode: JSON list of {out_dir, specs, inject_npz?, "
                         "inject_key?, inject_layer?, inject_alpha?, inject_alpha_frac?}. "
                         "Model loads ONCE; the injection hook is re-attached per cell. "
    ...
    if args.jobs_file is not None:
        ...
        # per cell (removing the previous one first — layers differ across cells). Each
        # gen re-seeds per spec, so cells are independent and byte-identical to the
        # single-cell path (verified by the dedicated bitwise smoke test).
    ...
    if args.spec_file is None or args.out_dir is None:
        raise SystemExit("single-cell mode requires --spec-file and --out-dir "
                         "(or --jobs-file for multi-cell)")
    handle, meta = _setup_injection(
